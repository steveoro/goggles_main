...
Feature: Workshops index for any swimmer
  As an anonymous user
  I want to be able to see the list of workshops for a specific swimmer
  And filter the list either by workshop date or name
  And browse to the details of each workshop

  Background: accessing the workshop list for a chosen swimmer does not require authentication
    Given I am not signed in
    And I have a chosen a random swimmer with existing user results
    When I visit the workshops page for the chosen swimmer
  ...
  Scenario: browsing the workshop list for a swimmer
