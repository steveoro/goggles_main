...
if (typeof window !== 'undefined' && !window.__chartJsRegistered) {
    Chart.register(...registerables)
    window.__chartJsRegistered = true
}
...
export default class extends Controller {
     ...
     */
    connect() {
        this.setChartStatus('pending')
        if (this.hasChartTarget && this.hasData1Value) {
            const chartType = this.hasTypeValue ? this.typeValue : 'pie'
            const Chart = resolveChartConstructor()
            if (!Chart) {
                this.setChartStatus('error', 'Chart constructor unavailable')
                return
            }
            ...
            try {
                this.chartInstance = new Chart(this.chartTarget, this.prepareConfig(chartType))
                this.setChartStatus('ready')
            } catch (error) {
                this.chartInstance = null
                this.setChartStatus('error', error && error.message ? error.message : 'Chart initialization failed')
            }
        }
    }
    ...
    setChartStatus(status, errorMessage = null) {
        this.element.dataset.chartStatus = status
        this.element.dataset.chartReady = status === 'ready' ? 'true' : 'false'
        if (errorMessage) {
            this.element.dataset.chartError = errorMessage
        } else {
            delete this.element.dataset.chartError
        }
    }
    // ---------------------------------------------------------------------------
