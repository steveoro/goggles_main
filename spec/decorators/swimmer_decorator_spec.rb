...
RSpec.describe SwimmerDecorator do
  let(:model_obj) { GogglesDb::Swimmer.limit(50).sample }
  subject { SwimmerDecorator.decorate(model_obj) }
  before(:each) do
    expect(model_obj).to be_a(GogglesDb::Swimmer).and be_valid
    expect(subject).to be_a(SwimmerDecorator).and be_valid
  end

  it_behaves_like('a paginated model decorated with', SwimmerDecorator)

  describe '#swimmer_age' do
    context 'with no parameters,' do
      it 'returns the current age of the swimmer' do
        expect(subject.swimmer_age).to eq(Date.today.year - model_obj.year_of_birth)
      end
    end
    context 'for a given date,' do
      it 'returns the age of the swimmer during that date\'s year' do
        sample_date = Date.today + (rand * 30 - rand * 15).to_i.years
  ...
  describe '#associated_team_ids' do
    context 'for a swimmer with existing badges,' do
      let(:result) { SwimmerDecorator.decorate(swimmer_with_badge).associated_team_ids }
    ...
    context 'for a swimmer without any badge,' do
      let(:result) { SwimmerDecorator.decorate(new_swimmer).associated_team_ids }
  ...
  describe '#link_to_teams' do
    ...
    context 'for a swimmer with existing badges,' do
      let(:decorated_obj) { SwimmerDecorator.decorate(swimmer_with_badge) }
    ...
    context 'for a swimmer without any badge,' do
      let(:result) { SwimmerDecorator.decorate(new_swimmer).link_to_teams }
  ...
  describe '#last_category_code' do
    context 'for a swimmer with existing badges,' do
      let(:result_code) { SwimmerDecorator.decorate(swimmer_with_badge).last_category_code }
    ...
    context 'for a swimmer without any badge,' do
      let(:result_code) { SwimmerDecorator.decorate(new_swimmer).last_category_code }
      it 'returns nil' do
  ...
  describe '#current_fin_category_code' do
    ...
    context 'for a swimmer without any badge,' do
      let(:decorated_obj) { SwimmerDecorator.decorate(new_swimmer) }
      let(:result_code) { decorated_obj.current_fin_category_code }
      let(:category_type) { GogglesDb::CategoryType.find_by(code: result_code) }
