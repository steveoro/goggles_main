...
RSpec.describe Solver::MeetingEvent, type: :strategy do
  context 'before #solve!,' do
    it_behaves_like('Solver strategy, bindings, finder & creator, before #solve!', 'MeetingEvent', Solver::MeetingEvent)
  end
  #-- -------------------------------------------------------------------------
  #++
    ...
    context "with solvable NEW #req data (valid w/ layout #{index})," do
      let(:fixture_row) do
        ...
        FactoryBot.build(
          ...
          begin_time: Time.now.in_time_zone('Europe/Rome').to_s
        )
      end
      let(:fixture_req) { req.call(fixture_row) }
