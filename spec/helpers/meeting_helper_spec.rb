...
RSpec.describe MeetingHelper do
  ...
  describe '#cache_key_for_meeting' do
    ...
    context 'without team_id & swimmer_id,' do
      subject(:result) { helper.cache_key_for_meeting(action: fixture_action, meeting_id: fixture_meeting_id, max_updated_at: max_updated_at) }

      it_behaves_like('#cache_key_for_meeting default behaviour')
    end

    context 'with a team_id,' do
      subject(:result) { helper.cache_key_for_meeting(action: fixture_action, meeting_id: fixture_meeting_id, max_updated_at: max_updated_at, team_id: team_id) }
    ...
    context 'with a swimmer_id,' do
      subject(:result) do
        helper.cache_key_for_meeting(action: fixture_action, meeting_id: fixture_meeting_id, max_updated_at: max_updated_at, swimmer_id: swimmer_id)
      end
    ...
    context 'with both a team_id & a swimmer_id,' do
      subject(:result) do
        helper.cache_key_for_meeting(action: fixture_action, meeting_id: fixture_meeting_id, max_updated_at: max_updated_at, team_id: team_id,
